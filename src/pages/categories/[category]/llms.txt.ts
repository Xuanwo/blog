...
export const GET: APIRoute = async ({ params }) => {
  ...
  const entry = taxonomies.categories[slug]
  ...
  const body = buildLlmsText({
    h1: `${site.title} / Category: ${entry.name}`,
    summary: `Posts in the "${entry.name}" category on ${site.baseURL}.`,
    details: [
      `HTML: ${toAbsoluteUrl(site.baseURL, `/categories/${slug}/`)}`,
